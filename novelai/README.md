...
ログには日時、状態変化、要求ID、モデル、キャラNO、行動、キャッシュID、API送信・画像保存・エラー詳細・設定変更を表示します。同じ状態を毎秒繰り返し表示しません。送信プロンプトの内容は `runtime/preview.json` で確認できます。APIキーは表示しません。

Emueraの入力待ち画面を外部から更新する機能は使っていません。生成画像は次の操作・タブ描画で自動反映します。その場で確認するには `[画像更新]` を押してください。生成待ち・失敗時は、条件に合う既存の一枚絵があれば表示します。
...
行動CSVにはコマンド・継続動作など341件を登録し、76件に初期タグを設定しました。残り265件はタグ空欄・`要設定`です。キーはゲーム内の名前との完全一致です。進行中の動作があれば優先し、なければ同じ対象・場所の直近コマンドを参照します。タグが空欄・未登録なら自然な待機姿勢にし、追加の文章生成AI・自動翻訳は使用しません。
...
幅・高さは各64～2048の64倍数、Stepsは1～50、scaleは0～10、CFG Rescaleは0～1です。Seed=-1で新規生成時にランダム化します。設定やタグの変更で送信内容が変わると別の画像キャッシュになります。

## 節約・再開・確認

- 同じプロンプトと生成パラメータは `resources/NovelAI/` の画像を再利用します。ゲームの要求IDや時刻だけでは再生成しません。
- 要求を積み上げず、待機後は最新の場面だけを生成します。古い場面の応答を現在の画像として表示しません。
- 通信失敗・中断は永続記録し、ワーカー再起動でも自動再送しません。設定の `[8] 失敗した生成を再試行` で明示的に再送します。結果不明の送信は既に課金済みの可能性があります。
...
APIキーと生成画像・通信ファイルはGit対象外です。`runtime` 全体を削除するとキー・送信記録も消えるため、画像だけを整理する場合は `resources/NovelAI` を対象にしてください。削除した画像を再生成するには設定画面の「再試行」を選択します。現在の個別キャラ設定は `characters.csv` に保存されています。
