...
最新更新: 2026-09-19。日本語で回答。既存実装を再調査せず、必要箇所とgit diffから再開する。

## 実装済み
- 2026-09-19: 継続行動優先の実装を削除。ERBは現在の相手・場所に対応するSELECTCOM_NAME履歴:0だけをaction行で送信し、継続DBは接触キャラ取得にだけ使用。ワーカーのModes参照も削除し、旧mode行は無視。プロンプト・画像名・ログは直近コマンド1件に統一、未実行時は待機。actorは先頭の操作キャラ、targetは最初の選択相手（最後の追加接触者ではない）。継続モード中の会話→散歩→未実行を実Emueraとワーカーのテストで確認。
- 2026-09-19: characters.csvに空欄のdefault_outfit列を追加。普段着はキャラNO別のこの列だけを参照し、空欄・未登録時は追加なし（編集先をログ表示）。着替え後はclothes.csvを使用。操作キャラにも適用。ゲームUIに新項目は追加せずCSV直接編集のみ、既存の外見編集で新列を保持。clothes.csvの普段着行を削除し、出力時も除外。画像名は従来どおりでタグ編集後は再生成が必要。
- 2026-09-19: ワーカーログを全体・キャラ別に整理。New-Payloadで組み立てた服装名・タグをWrite-PromptLogで独立表示し、操作キャラ/相手・名前・NOも表示。未受信/空欄の理由を明記。V3/V4/V5・Forge・ComfyUIで共通。ネガティブはコンソールに出さずAPIとpreview.jsonに保持。画像再利用は「API送信なし」と表示。
- 2026-09-18: プロンプトと識別キー以外のCSV列を削除。服装は`name,prompt`、行動は`name,scene,actor,target`。出力スクリプトもsource/statusを生成しない。旧形式の読み込みは許可し、編集済みタグ・空欄は保持。`-FillMissingTags`を明示した場合だけ空欄の行動タグを補完する。
- 2026-09-17: `novelai/clothes.csv`は共通衣装のみ117件（英語初期タグ91件）。`ERB/通常衣装関連`だけを抽出し、キャラCSV・個別口上ERBの専用衣装は既存行からも除外。固有の普段着名と番号指定衣装は「普段着」扱い。NAI_キャラ行から現在のTEQUIP/CSTRを送り、操作キャラと相手それぞれのプロンプトへ服装を追加。画像名は操作キャラから`{NO}_{服装}_{NO}_{服装}_{行動}.png`。服装の追加名・脱衣状態は+連結、着替えで別キャッシュになる。命名・キャッシュ検索はGet-ImageNameに一本化し、ゲームは現在の要求IDの応答名で読む。旧画像は保持し新キャッシュには流用しない。`export-clothes.ps1`・`export-csv.cmd`で更新可能、共通服の編集は保持。test-clothesで実Emueraの抽出・操作キャラ切替・新画像読込・脱衣後の古い応答除外、test.ps1で服別保存・再利用を検証済み。旧test-renderは現行にない関数へ依存するため表示はtest-controlsを使用。
...
- Write-Logで日時付き状態・場面/モデル/行動・API送信/保存/エラー・設定更新を表示。同一状態は繰り返さず、エラー詳細のAPIキーは伏せる。送信時に全体・キャラ別の正プロンプトと服装を全文表示。preview.jsonも維持。
- 直近コマンドはaction行で送信し、継続動作DBは接触キャラの取得にだけ使用。有効な直近コマンドがなければ待機。ERB反映にはゲームの保存・再起動が必要。
