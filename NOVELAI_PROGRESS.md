...
最新更新: 2026-09-13。日本語で回答。既存実装を再調査せず、必要箇所とgit diffから再開する。
...
## 運用・残る確認
- 現在のブランチはauto-image。待機項目・全プロンプトログ・直近行動の変更は未コミット。characters.csv・prompts.csv・actions.csvのユーザー編集（待機scene空欄など）は保持。テストは待機の空欄を許可する。
